  ...
  (config) => {
    ...
    try {
      ...
      if (user?.token) {
       ...
       console.error("Error parsing user token in api.js", error);
    }
    return config;
  },
  (error) => {
...
// Response Interceptor to support connection fallback on error
API.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (isNetworkError && originalRequest && !originalRequest._isRetry) {
      ...
      console.warn("Backend offline in api.js. Dynamically redirecting request to Standalone Sandbox Adapter...");
